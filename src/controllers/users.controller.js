...
const registerUser = async (req, res) =>{
    ...
    await connection.query(`SELECT nombres FROM usuarios WHERE email = ${connection.escape(userBody.email)};`, async (error, result, fields) =>{
        if(result.length === 0){
            try {
                await connection.query(`Insert into usuarios (nombres, apellidos, fecha_nacimiento, email, genero, tipo_documento, identificacion, telefono, rol, estado) values (${connection.escape(userBody.nombres)}, ${connection.escape(userBody.apellidos)}, ${connection.escape(userBody.fecha_nacimiento)}, ${connection.escape(userBody.email)}, ${connection.escape(userBody.genero.charAt(0))}, ${connection.escape(userBody.tipo_documento)}, ${connection.escape(userBody.identificacion)}, ${connection.escape(userBody.telefono)}, ${connection.escape(userBody.rol)}, 'A')`, async (error, result, fields) =>{
                    if(!error){
